...
3. **Полётные сценарии и диагностика** (корень репозитория) — собирают слои
   ...
   - `diagnosis.py` — `run_diagnosis`: те же ROS-подсистемы (`get_telemetry`
     в кадрах `body`/`aruco_map`, видимость меток `aruco_detect/markers`,
     дальномер, камера+`StationVision`, LED, опционально захват), но
     **никогда не вызывает** `navigate()`/`arming(True)`/`land()` — двигатели
     не включаются. Каждая проверка обёрнута в `_run_check`, изолирующий её
     исключение в `CheckResult(ok=False, ...)`, чтобы падение одной проверки
     не скрывало результаты остальных — это единственное существенное
     отличие от `run_mission()` в `bvs*.py`, где падение шага должно
