...
def run_station(
    *,
    ...
    ``channel`` — ``station_link.StationChannel`` или ``None``. Связь по сети
    НЕ подменяет опознание по цвету: регламент требует, чтобы станция сама
    находила нужный БВС по ленте (5 баллов), и приглашение зажигается по
    цвету, как раньше. UDP добавляет две вещи, которых цвет дать не может:
    адресность (кому именно ответить ``landing_granted``) и обратную связь —
    борт докладывает ``landed``/``departed``, и станция знает, занята она или
    свободна, а не гадает по таймеру.
    """
    ...
    def pump(seconds: float) -> None:
        """Прокачать связь ``seconds`` секунд, разбирая доклады борта.

        Приём у нас явный, без потоков: пока станция спит между кадрами,
        входящие датаграммы никто не читает, и очередь копится. Поэтому все
        паузы цикла идут через эту функцию, а не через голый ``sleep``.

        Пауза выдерживается целиком: ``channel.pump`` возвращается сразу, как
        только пришла датаграмма, и без внешнего дедлайна разговорчивый борт
        схлопнул бы пятнадцать секунд зарядки в доли секунды.
        """
        if channel is None:
            if seconds > 0:
                sleep_fn(seconds)
        ...
        while True:
            ...
            for message in channel.pump(max(0.0, left)):
                event_writer(
                    "link_message", {"kind": message.kind, "from": message.sender}
                )
            if left <= 0:
                return
    ...
    while not should_stop():
        ...
        if detection is not None and streak_count >= config.required_frames:
            # Кто именно просит посадку. None — борт молчит по сети (или
            # связи нет вовсе): приглашение всё равно зажигаем, это требование
            # регламента и оно не должно зависеть от Wi-Fi.
            asking = channel.pending() if channel is not None else None

            grant_sender(detection)
            grants += 1
            if asking is not None:
                channel.grant(asking)
            event_writer(
                "landing_granted",
                {
            ...
            while absent_frames < config.reset_frames and not should_stop():
                ...
                pump(config.poll_interval_s)

            if asking is not None and channel is not None:
                # Борт улетел (или мы решили, что улетел) — забываем его,
                # иначе протухший запрос будет держать станцию «занятой».
                channel.forget(asking)
            streak_color = None
            streak_count = 0
...
def _self_test_link() -> None:
    """Проверить связь: борт просит посадку — станция отвечает именно ему.

    Сеть подменена на память (``LoopbackNetwork`` из station_link), поэтому
    проверка идёт без сокетов, роутера и Raspberry Pi.
    """
    link_module = importlib.import_module("station_link")

    network = link_module.LoopbackNetwork()
    station = link_module.StationChannel(
        link_module.Link(
            "station1",
            network.transport("10.0.0.21"),
            peers={"uav1": "10.0.0.11"},
        )
    )
    drone = link_module.DroneChannel(
        link_module.Link(
            "uav1",
            network.transport("10.0.0.11"),
            peers={"station1": "10.0.0.21"},
        ),
        station="station1",
    )

    # Пока борт молчит, приглашать некого.
    station.pump()
    assert station.pending() is None

    drone.link.send(link_module.Kind.LANDING_REQUEST)
    station.pump()
    assert station.pending() == "uav1", "запрос борта не дошёл до станции"

    station.grant("uav1")
    answered = [message.kind for message in drone.link.pump()]
    assert link_module.Kind.LANDING_GRANTED in answered, "борт не получил разрешение"

    # Доклады борта: станция должна понимать, занята она или свободна.
    drone.report_landed()
    station.pump()
    ...
    assert station.pending() is None, "после отлёта запрос должен сниматься"
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    # Связь с бортами по UDP (lib/station_link.py). Без --drone станция
    # работает как раньше, только по цвету.
    parser.add_argument(
        "--id",
        default="station1",
        help="как станция представляется бортам; борт шлёт датаграммы этому имени",
    )
    parser.add_argument(
        "--drone",
        action="append",
        default=[],
        metavar="ИМЯ=IP",
        help="борт и его адрес, можно повторять: --drone uav1=192.168.0.11",
    )
    parser.add_argument(
        "--port",
        type=int,
        default=5801,
        help="порт UDP, одинаковый с обеих сторон",
    )
    return parser


def _open_link(args: Any) -> Any:
    """Поднять UDP-связь с бортами. ``None`` — работаем только по цвету.

    Отказ связи не должен ронять станцию: без сети она всё равно обязана
    опознавать борт по ленте и приглашать его — за это дают 5 баллов, и они
    не зависят от роутера.
    """
    if not args.drone:
        return None

    peers: Dict[str, str] = {}
    for item in args.drone:
        if "=" not in item:
            raise SystemExit("--drone ждёт ИМЯ=IP, получено {!r}".format(item))
    ...
    try:
        ...
        print("связь выключена: нет lib/station_link.py ({})".format(error))
        return None

    try:
        link = link_module.Link(
            args.id,
            link_module.UdpTransport(port=args.port),
            peers=peers,
        )
    except (RuntimeError, OSError) as error:
        # UdpTransport поднимает RuntimeError на занятом порту.
        print("связь выключена: порт {} занять не вышло ({})".format(args.port, error))
        return None
    return link_module.StationChannel(link)
