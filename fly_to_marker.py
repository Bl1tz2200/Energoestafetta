...
3. Поставить дрон на площадку «Н» (метка 48) и запустить::
       ...
       python3 fly_to_marker.py        # без аргумента — тоже к метке 5

Первая строка вывода — «метка 5 -> точка (6.0, 1.0)» — печатается ДО взлёта.
Если точка не та, жать Ctrl+C, дрон ещё стоит на земле.
...
print("метка {} -> точка ({}, {})".format(target, target_x, target_y))
