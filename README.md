...
эту же строку в конец `~/.bashrc` на нём и **открыть новый терминал/SSH-сессию**
(`.bashrc` читается один раз при старте шелла — уже открытая сессия правку не
подхватит; либо выполнить `source ~/.bashrc` в текущей).
