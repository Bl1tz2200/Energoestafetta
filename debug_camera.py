...
* **масштаб** — замеренный по кадру шаг решётки должен совпасть с картой.
  Масштаб считается через фокусное расстояние, поэтому некалиброванная камера
  или неверный ``--marker-size`` дают систематическую ошибку во ВСЕХ
  расстояниях: «миллиметровые» поправки становятся метровыми;
