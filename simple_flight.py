...
* обход зоны крестом при потере позиции, сторож деревьев в полёте, геозона,
  удержание курса, дизарм через ``mavros/cmd/arming``, режим ``--probe``.

Что осталось, потому что без этого нельзя:

* подъём «осмотреться», если карты не видно: метка 48 в углу поля и закрыта
  площадкой «Н», соседей у неё только с двух сторон, и на рабочей высоте их в
  кадре может не набраться. Рабочая высота при этом растёт вместе с дроном;
...
# Если карты так и не видно — поднимаемся: с высоты в кадр попадает больше
# меток. Особенно это нужно над стартом: метка 48 в углу поля и закрыта
# площадкой «Н», соседи у неё только с двух сторон, и на рабочей высоте их
# может не набраться.
CLIMB_LOOK = 0.5      # на сколько подниматься за раз, м
MAX_ALT = 3.5         # выше не поднимаемся: потолок полётной зоны 4 м
...
def wait_position():
    """Дождаться позиции, при нужде поднявшись повыше.

    ``None`` — не помогло и это, дальше лететь нельзя.
    """
    global ALT

    # Сначала просто ждём: карта часто возвращается сама, метки уходят из кадра
    # на кочке и при бликах.
    ...
    # Не вернулась. Поднимаемся: с высоты в кадр попадает больше меток. Рабочая
    # высота при этом растёт вместе с нами — спускаться обратно туда, откуда
    # карты не видно, смысла нет, оттуда мы её только что и не увидели.
    while ALT < MAX_ALT:
        ALT = min(ALT + CLIMB_LOOK, MAX_ALT)
        print("  поднимаемся осмотреться, рабочая высота теперь {:.1f} м".format(ALT))
        navigate(x=0.0, y=0.0, z=CLIMB_LOOK, yaw=0.0, speed=DESCENT_SPEED,
                 frame_id="body")
        time.sleep(CLIMB_LOOK / DESCENT_SPEED + PAD)

        where = position()
        if where is not None:
            print("  карта нашлась на {:.1f} м".format(ALT))
            return where

    print("  карты не видно даже с {:.1f} м".format(ALT))
    return None
...
def main():
    ...
    parser.add_argument("--start-marker", type=int, default=41,
