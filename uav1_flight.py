...
class MissionConfig:
    ...
    # Набор высоты — БЫСТРЕЕ горизонтали, вопреки первому впечатлению. На земле
    # дрон закрывает стартовую метку собой, соседние в кадр с пола не попадают,
    # и до высоты захвата карты PX4 держит точку вслепую: без VPE фильтр
    # интегрирует смещение акселерометра, и оценка места уползает. Дрейф при
    # этом растёт как t², то есть решает не мягкость взлёта, а его КОРОТКОСТЬ:
    # 2026-08-01 при 0.3 м/с дрон уехал на 1.5 м, и втрое более быстрый набор
    # сокращает этот увод примерно на порядок. Мягкость взлёта важна была,
    # пока считалось, что виновата раскачка наверху, — замеры показали другое.
...
def _run_mission_by_map(pilot: Pilot, route: Sequence[int]) -> bool:
    ...
    # ВАЖНО: на земле этой оценке верить нельзя, и по ней ничего не решаем.
    # Дрон стоит на стартовой метке и закрывает её, камера в десятке
    # сантиметров от пола видит в лучшем случае кусок одной соседней метки под
    # острым углом — позиция из такого кадра уезжает на метр и больше. Это
    # ровно тот случай, когда «проверка» отсеивала бы исправные запуски.
    # Настоящая сверка — после взлёта, с рабочей высоты.
    pose = pilot.read_map_pose()
    if pose is None:
        pilot.say(">>> НА ЗЕМЛЕ карта не видна (дрон закрывает стартовую метку) — "
                  "это норма, сверимся после взлёта")
    else:
        error = math.hypot(pose[0] - start_xy[0], pose[1] - start_xy[1])
        pilot.say(">>> НА ЗЕМЛЕ по aruco_map: ({:.2f}, {:.2f}, {:.2f}), старт {} в "
                  "({:.2f}, {:.2f}), расхождение {:.2f} м — с пола оценке не верим, "
                  "сверимся после взлёта".format(
